...
//            print(grid);
    ...
    private static void stabilize(char[][] grid, Queue<Change> changes) {
        for (int y = 0; y < grid.length; y++) {
            for (int x = 0; x < grid[y].length; x++) {
                if (grid[y][x] == 'L' && countVisibleOccupied(grid, y, x) == 0) {
                    changes.add(Change.to(y, x, '#'));
                } else if (grid[y][x] == '#' && countVisibleOccupied(grid, y, x) >= 5) {
    ...
    private static int countVisibleOccupied(char[][] grid, int y, int x) {
        int occupied = 0;
        outer: for (int[] offset : OFFSETS) {
            int largestMagnitude = Math.max(grid.length, grid[0].length);
            for (int magnitude = 1; magnitude <= largestMagnitude; magnitude++) {
                try {
                    char at = grid[y + offset[1] * magnitude][x + offset[0] * magnitude];
                    if (at == '#') {
                        occupied += 1;
                        continue outer;
                    } else if (at == 'L') {
